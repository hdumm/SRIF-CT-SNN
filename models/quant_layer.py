...
class QuantAvg(nn.AvgPool2d):
    def __init__(self, kernel_size, stride):
        ...
        # self.membrane_init = torch.nn.Parameter(torch.tensor(0.5))
...
class QuantReLU(nn.ReLU):
    def __init__(self, inplace: bool = False):
        ...
        self.act_grid = build_power_value(self.bit, additive=True)
        self.act_alq = act_quantization(self.bit, self.act_grid, power=True)
        self.act_alpha = torch.nn.Parameter(torch.tensor(2.5))
...
class QuantSigmoid(nn.Sigmoid):
    def __init__(self, inplace: bool = False):
        super(QuantSigmoid, self).__init__(inplace)
        self.layer_type = 'QuantSigmoid'
        self.bit = 4
        self.act_grid = build_power_value(self.bit, additive=True)
        self.act_alq = act_quantization(self.bit, self.act_grid, power=True)
        self.act_alpha = torch.nn.Parameter(torch.tensor(8.0))
    ...
    def forward(self, x):
        x = F.Sigmoid(x, inplace=self.inplace)
        return self.act_alq(x, self.act_alpha)
...
class QuantSeparableConv2D(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0):
        super(QuantSeparableConv2D, self).__init__()

        self.depthwise = QuantConv2d(in_channels, in_channels, kernel_size, stride, padding, groups=in_channels)
        self.pointwise = QuantConv2d(in_channels, out_channels, 1)
...
class QuantDepthwiseConv2D(nn.Module):
    def __init__(self, in_channels, kernel_size, stride=1, padding=0):
        super(QuantDepthwiseConv2D, self).__init__()
