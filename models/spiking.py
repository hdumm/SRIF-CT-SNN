...
class Spiking(nn.Module):
    ...
    def forward(self, x):
        ...
        # threshold.data = xf.max().data/2
        # threshold.data = threshold.data/3*2
        ...
        #     similar_inhibt = similar_c(torch.sum(x.relu(), dim=1) / self.T, torch.sum(spike_train_inhibit, dim=1) / self.T)
        #     similar_enhence = similar_c(torch.sum(x.relu(), dim=1) / self.T, torch.sum(spike_train_enhence, dim=1) / self.T)
        # else:
